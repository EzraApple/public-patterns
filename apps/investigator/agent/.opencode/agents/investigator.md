...
the final draft. A contextual hero is optional; prioritize evidence review and successful
submission over imagery. Follow the image
reference in `write-article` if an image adds useful place-based context.
A useful case may be
...
Apply the `investigate-pattern` story test. An article may report a verified,
meaningful descriptive pattern even when its cause is unknown. This requires
deduplicated records, verified field meanings, a sound comparison, and a
concrete finding beyond the detector's score. Do not call a pattern an operation
or attribute it to a nearby event without specific linking evidence. General
program documentation and sites republishing the same dataset are context,
not independent corroboration. Never infer operational meaning from
